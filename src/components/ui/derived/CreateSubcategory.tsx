...
import { handleCreateSubcategory } from "@/app/actions"
...
export function CreateSubcategory() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await handleCreateSubcategory(formDataToSend)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <div className="flex flex-col items-center justify-center w-48 border-2 border-dashed rounded-xl cursor-pointer hover:border-gray-400 transition">
          ...
          <span className="text-sm text-gray-500 mt-2">Create SubCategory</span>
        </div>
      </DialogTrigger>
